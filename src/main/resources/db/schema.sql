...
(
    ...
    CONSTRAINT fk_stored_file_metadata_bucket_id FOREIGN KEY (bucket_id) REFERENCES tb_bucket(id)
        ON DELETE CASCADE,
...
(
    id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    access_key TEXT NOT NULL UNIQUE CHECK ( length(access_key) BETWEEN 3 AND 64) CHECK ( access_key GLOB '[A-Za-z0-9_-]*'),
...
(
    ...
    status TEXT NOT NULL CHECK (status IN ('ACTIVE', 'ROTATING', 'INACTIVE')),
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_buckets_access_key_id ON tb_bucket(access_key_id);
